...
export default function InterviewPrep({ isDarkMode }) {
  ...
  // --- LOGIC: EXPORT LIVE PREP SNAPSHOTS VIA BLOB STREAM ---
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      link.href = blobUrl;
      link.setAttribute("download", `${activeSection}_Preparation_Guide.pdf`);
      document.body.appendChild(link);
      link.click();
      link.remove();
    } catch (err) {
      console.error("PDF Exporter boundary failure:", err);
      alert("Could not extract active roadmap binary file streams.");
    }
  };
  ...
  const btnPrimary = { padding: "18px 32px", backgroundColor: "var(--primary-accent)", color: "white", borderRadius: "50px", border: "none", fontWeight: "900", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: "10px", width: "100%" };
  ...
  return (
    <div style={{ maxWidth: "1000px", margin: "0 auto", padding: "20px", minHeight: "70vh" }}>
      
      {/* HEADER SECTION LAYOUT */}
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} style={{ textAlign: "center", marginBottom: "40px" }}>
        <h1 style={{ fontSize: "3rem", fontWeight: "900", color: "var(--body-text)", letterSpacing: "-2px" }}>
      ...
      <AnimatePresence mode="wait">
        ...
        {!isAuthenticated && (
          <motion.div key="locked" initial={{ opacity: 0, y: -30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }}
            style={{ maxWidth: "600px", margin: "0 auto", padding: "60px 40px", borderRadius: "30px", backgroundColor: "var(--card-bg)", border: `1px solid ${isDarkMode ? "rgba(255,255,255,0.1)" : "#1c2c45"}`, boxShadow: "0 25px 50px rgba(0,0,0,0.15)", textAlign: "center" }}
          >
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "15px" }}>
              <button style={btnPrimary} onClick={() => navigate("/login", { state: { from: location.pathname, targetRole: role, experience: experience } })}><LogIn size={20} /> Login to Unlock</button>
              <button style={btnSecondary} onClick={() => navigate("/signup", { state: { from: location.pathname, targetRole: role, experience: experience } })}><UserPlus size={20} /> Create an Account</button>
            </div>
          </motion.div>
        )}

        {/* STATE 2: CORE TRAINING HUB SELECTION DASHBOARD */}
          ...
          >
            {[
              ...
              <motion.div 
                ...
                style={{ padding: "40px", borderRadius: "30px", cursor: "pointer", backgroundColor: "var(--card-bg)", border: `1px solid ${isDarkMode ? "rgba(255,255,255,0.1)" : "#1c2c45"}`, color: "white", boxShadow: "0 10px 30px rgba(0,0,0,0.1)", textAlign: "left" }}
              >
                <div style={{ color: "var(--primary-accent)", marginBottom: "20px" }}>{card.icon}</div>
                <h3 style={{ fontSize: "1.5rem", fontWeight: "900", marginBottom: "10px" }}>{card.title}</h3>
        ...
        {/* STATE 3: LIVE RE-RENDER ENGINE (GD VS QUESTION FLASHCARDS) */}
        {isAuthenticated && activeSection && (
              ...
              content && (
                <div style={{ display: "flex", flexDirection: "column", gap: "30px" }}>
                  ...
                  {activeSection === "GD" && content.map(topic => (
                    <div key={topic.id} style={{ backgroundColor: "var(--card-bg)", borderRadius: "20px", padding: "30px", border: `1px solid ${isDarkMode ? "rgba(255,255,255,0.1)" : "#1c2c45"}`, color: "white" }}>
                      <h3 style={{ fontSize: "1.5rem", fontWeight: "900", color: "var(--primary-accent)", marginBottom: "30px", textAlign: "center" }}>Topic: {topic.topic}</h3>
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "30px", textAlign: "left" }}>
                        <div style={{ backgroundColor: "rgba(34, 197, 94, 0.05)", padding: "20px", borderRadius: "15px", border: "1px solid rgba(34, 197, 94, 0.2)" }}>
                          ...
                          <ul style={{ display: "flex", flexDirection: "column", gap: "12px", opacity: 0.9, paddingLeft: "15px", margin: 0, lineHeight: "1.5" }}>
                  ...
                  {/* --- COMPONENT VIEW B: TACTICAL INTERVIEW FLASHCARD ACCORDIONS --- */}
                  {activeSection !== "GD" && content.map((item, index) => (
                    <div key={item.id || index} style={{ backgroundColor: "var(--card-bg)", borderRadius: "20px", border: `1px solid ${isDarkMode ? "rgba(255,255,255,0.1)" : "#1c2c45"}`, overflow: "hidden", textAlign: "left" }}>
                      <div onClick={() => setActiveFlashcard(activeFlashcard === item.id ? null : item.id)} style={{ padding: "25px", cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <h3 style={{ fontSize: "1.2rem", fontWeight: "800", color: "white", margin: 0, paddingRight: "20px" }}>Q{index + 1}: {item.q}</h3>
                      ...
                      <AnimatePresence>
                        {activeFlashcard === item.id && (
                          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} style={{ borderTop: "1px solid rgba(255,255,255,0.1)", backgroundColor: "rgba(0,0,0,0.2)" }}>
                            <div style={{ padding: "25px" }}>
                              <h4 style={{ color: "#22c55e", marginBottom: "10px", fontWeight: "bold", display: "flex", alignItems: "center", gap: "8px" }}><BrainCircuit size={18} /> Ideal AI Suggested Answer</h4>
                              <p style={{ color: "rgba(255,255,255,0.8)", lineHeight: "1.7", margin: 0 }}>{item.a}</p>
                            </div>
                          </motion.div>
                        )}
                      </AnimatePresence>
