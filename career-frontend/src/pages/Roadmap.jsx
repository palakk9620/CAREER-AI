...
export default function Roadmap({ isDarkMode }) {
    ...
    const handleGenerate = async (e) => {
        ...
        // Grab username from session fallback storage to map database lookups
        ...
        try {
            ...
            // Map data directly from backend schema array structure
            setRoadmapData({
                role: targetRole,
                duration: duration,
    ...
    // --- LOGIC: DOWNLOAD LIVE EXPORTER BLOB FROM API ---
    const handleDownloadPDF = async () => {
        ...
        try {
            ...
            link.href = blobUrl;
            link.setAttribute("download", `Career_Roadmap_${targetRole}.pdf`);
            document.body.appendChild(link);
            link.click();
            link.remove();
        } catch (err) {
            console.error("PDF download failed:", err);
            alert("Failed to generate PDF document snapshot.");
        }
    };
    ...
    return (
        <div style={{ maxWidth: "1000px", margin: "0 auto", padding: "20px", minHeight: "70vh", display: "flex", flexDirection: "column", justifyContent: "center" }}>

            <motion.h1 initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} style={{ fontSize: "3rem", fontWeight: "900", marginBottom: "30px", color: "var(--body-text)", letterSpacing: "-2px", textAlign: "center" }}>
            ...
            <AnimatePresence mode="wait">
                ...
                {!isAuthenticated && (
                    ...
                    >
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: "15px" }}>
                            <button
                                style={btnPrimary}
                                onClick={() => navigate("/login", { state: { from: location.pathname, targetRole: targetRole, experience: location.state?.experience } })}
                            >
                            ...
                            <button
                                style={btnSecondary}
                                onClick={() => navigate("/signup", { state: { from: location.pathname, targetRole: targetRole, experience: location.state?.experience } })}
                            >
                ...
                {isAuthenticated && !roadmapData && (
                    <motion.form key="setup" initial={{ opacity: 0, y: -30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} onSubmit={handleGenerate}
                        style={{ maxWidth: "800px", margin: "0 auto", padding: "50px", borderRadius: "30px", backgroundColor: "var(--card-bg)", border: `1px solid ${isDarkMode ? "rgba(255,255,255,0.1)" : "#1c2c45"}`, boxShadow: "0 25px 50px rgba(0,0,0,0.15)" }}
                    >
                ...
                {isAuthenticated && roadmapData && (
                    <motion.div key="report" initial={{ opacity: 0, y: -30 }} animate={{ opacity: 1, y: 0 }}>
                        ...
                        <div style={{ textAlign: "center", marginBottom: "30px" }}>
                            <h2 style={{ fontSize: "2rem", color: "var(--body-text)", fontWeight: "900" }}>{roadmapData.duration} Plan for <span style={{ color: "var(--primary-accent)" }}>{roadmapData.role}</span></h2>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px", marginBottom: "30px", textAlign: "left" }}>
                            <div style={{ padding: "25px", borderRadius: "20px", backgroundColor: "var(--card-bg)", border: "1px solid rgba(255,255,255,0.05)" }}>
                                ...
                                <p style={{ fontSize: "0.95rem", opacity: 0.8 }}>{roadmapData.currentSkills.join(", ") || "Analyzed from resume."}</p>
                            </div>
                            <div style={{ padding: "25px", borderRadius: "20px", backgroundColor: "var(--card-bg)", border: "1px solid rgba(255,255,255,0.05)" }}>
                                <h4 style={{ color: "var(--primary-accent)", marginBottom: "10px", fontWeight: "bold" }}>Target Gaps to Bridge</h4>
                                <p style={{ fontSize: "0.95rem", opacity: 0.8 }}>{roadmapData.missingSkills.join(", ") || "Mapped by Gemini."}</p>
                            </div>
                        </div>

                        {/* PATHWAY CONTAINER */}
                        <div style={{ padding: "40px", borderRadius: "30px", backgroundColor: "var(--card-bg)", border: `1px solid ${isDarkMode ? "rgba(255,255,255,0.1)" : "#1c2c45"}`, marginBottom: "40px", color: isDarkMode ? "white" : "#0f172a", textAlign: "left" }}>
                            {roadmapData.milestones.map((node, index) => (
                                <div key={index} style={{ marginBottom: index === roadmapData.milestones.length - 1 ? 0 : "40px", borderLeft: "3px solid var(--primary-accent)", paddingLeft: "20px" }}>
                                    ...
                                    <p style={{ opacity: 0.9, lineHeight: "1.6", fontSize: "1.05rem" }}><strong>Core Focus:</strong> {node.know_how}</p>
                                    <p style={{ opacity: 0.8, marginTop: "6px", fontSize: "0.95rem" }}><strong>Hands-on Build:</strong> {node.project}</p>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px" }}>
                            ...
                            <motion.div whileHover={{ y: -5, backgroundColor: isDarkMode ? "rgba(255,255,255,0.08)" : "#f1f5f9" }} whileTap={{ scale: 0.98 }} onClick={handleDownloadPDF} style={{ padding: "30px", borderRadius: "20px", cursor: "pointer", backgroundColor: "var(--card-bg)", border: `1px solid ${isDarkMode ? "rgba(255,255,255,0.1)" : "#1c2c45"}`, color: isDarkMode ? "white" : "#0f172a", display: "flex", alignItems: "center", gap: "20px", textAlign: "left" }}>
                                ...
                                <div><h3 style={{ fontSize: "1.2rem", fontWeight: "900" }}>Download PDF</h3><p style={{ fontSize: "0.9rem", opacity: 0.8 }}>Save roadmap offline.</p></div>
                            </motion.div>
                        </div>
